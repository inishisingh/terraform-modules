...
resource "azurerm_monitor_data_collection_rule" "this" {
  name                        = "${var.location_cli}-${var.environment}-${var.workload}-${var.department}-dcr-windows-am"
  ...
  dynamic "data_sources" {
    for_each = length(var.performance_counters) > 0 || length(var.windows_event_logs) > 0 || length(var.iis_logs) > 0 || length(var.log_files) > 0 || length(var.extensions) > 0 ? [1] : []
    content {
