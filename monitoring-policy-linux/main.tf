...
provider "azurerm" {
  features {}
    ...
    resource "azurerm_policy_definition" "ama_by_tag" {
      ...
      display_name = "Deploy AMA + DCR Association for Linux VMs with tag"
      description  = "Installs Azure Monitor Agent on Linux VMs and associates them to a DCR when a tag matches allowed values"
      policy_type  = "Custom"
      mode         = "Indexed"
